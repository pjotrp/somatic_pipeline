...
while read ref tumor ; do
  ...
  refout=${refname##*/}
    ...
    echo "$inheritance -d -in $tumorout.bamAnn -out $tumorout -uaf_ids 3 -aff_ids 4 -min_cov 8"|~/izip/git/opensource/ruby/once-only/bin/once-only --pfff -d $dir -v --ignore-lock
done < bamfiles_2.txt
